...
export function ContactSection() {
  ...
  const getRecaptchaToken = async () => {
    ...
    if (!siteKey || !grecaptcha?.execute) return null
    try {
  ...
  const onSubmit = (values: FormValues) => {
    startTransition(async () => {
      ...
      if (!token) {
        toast({ title: "No se pudo validar reCAPTCHA", description: "Recarga la página e intenta de nuevo." })
        return
      }
      ...
      if (res.ok) {
        ...
        toast({ title: "reCAPTCHA falló", description: "Parece tráfico automático. Intenta de nuevo." })
      } else if (res.error === "validation") {
        toast({ title: "Revisa el formulario", description: "Hay campos con errores." })
      } else {
        toast({ title: "Ups, no se pudo enviar", description: "Intenta nuevamente en unos minutos." })
      }
    })
  }
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div>
              ...
              <p className="text-lg text-muted-foreground leading-relaxed text-pretty">{t.contactDescription}</p>
            </div>
          ...
          <Card className="bg-background">
            ...
            <CardContent className="space-y-4">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                  <Textarea placeholder={t.yourMessage} className="min-h-[120px]" {...register("message")} aria-invalid={!!errors.message} />
                  {errors.message && <p className="mt-1 text-xs text-destructive">{errors.message.message}</p>}
                </div>

                <Button className="w-full" size="lg" type="submit" disabled={isPending}>
                  {isPending ? "Sending..." : t.sendMessageBtn}
                </Button>
              </form>
            </CardContent>
          </Card>
